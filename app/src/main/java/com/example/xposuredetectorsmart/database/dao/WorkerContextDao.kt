...
interface WorkerContextDao {
    ...
    @Query("SELECT * FROM worker_context ORDER BY scanTimestamp DESC LIMIT 1")
    fun observeLatestContext(): Flow<WorkerContext?>

    @Query("SELECT * FROM worker_context ORDER BY scanTimestamp DESC LIMIT 1")
    suspend fun getLatestContext(): WorkerContext?

    @Query("SELECT * FROM worker_context WHERE workerId = :workerId ORDER BY scanTimestamp DESC LIMIT 1")
    suspend fun getLatestForWorker(workerId: String): WorkerContext?

    @Query("SELECT DISTINCT workerId FROM worker_context ORDER BY scanTimestamp DESC")
